...
// boolToString()
const char *boolToString(boolean value) { return value ? "true" : "false"; }
...
void getState() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    myData.cpu = String(ESP.getChipModel()) + "(v" +
                 String(ESP.getChipRevision()) + ")" + " CPU" +
...
boolean connectToMqtt() {
  ...
  if (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(hostname.c_str(), lastWillTopic.c_str(), 1, true,
                           "offline")) {
      Serial.println(" OK");
...
#endif
#if defined(ESP32)
  uint8_t fragmentationPercentage = static_cast<uint8_t>(
      (100 * (myData.memfree - ESP.getMaxAllocHeap())) / myData.memfree);
...
void initWebSocket() {
  ...
  server.on("/js/bootstrap.bundle.min.js", HTTP_GET,
            [](AsyncWebServerRequest *request) {
              request->send(LittleFS, "/js/bootstrap.bundle.min.js",
                            "text/javascript");
            });
